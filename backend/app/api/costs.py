...
# Mock data
COSTS_DATA = [
    {"id": "1", "service": "EC2", "amount": 2450.50, "currency": "USD", "date": datetime.now() - timedelta(days=1), "account_id": "123456789012", "region": "us-east-1", "trend": "up", "trend_percent": 12},
    {"id": "2", "service": "S3", "amount": 580.25, "currency": "USD", "date": datetime.now() - timedelta(days=1), "account_id": "123456789012", "region": "us-east-1", "trend": "down", "trend_percent": 8},
    {"id": "3", "service": "Lambda", "amount": 120.75, "currency": "USD", "date": datetime.now() - timedelta(days=1), "account_id": "123456789012", "region": "us-east-1", "trend": "neutral", "trend_percent": 0},
    {"id": "4", "service": "RDS", "amount": 890.30, "currency": "USD", "date": datetime.now() - timedelta(days=1), "account_id": "123456789012", "region": "us-east-1", "trend": "up", "trend_percent": 5},
]
...
async def analyze_costs(account_id: str = Query(...)):
    ...
    return {
        "recommendations": [
            {"title": "Reserved Instances", "savings": 400, "description": "Save 40% on EC2 with 1-year commits", "priority": "high"},
            {"title": "Idle Resources", "savings": 120, "description": "Terminate unused instances", "priority": "high"},
            {"title": "Storage Optimization", "savings": 150, "description": "Archive old S3 objects to Glacier", "priority": "medium"},
        ],
...
async def detect_cost_anomalies(
    ...
    threshold_percent: float = Query(30)
):
    """Detect cost anomalies for an account"""
    logger.info(f"Detecting anomalies for account {account_id} with threshold {threshold_percent}%")
