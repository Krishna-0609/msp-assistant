...
class AWSService:
    ...
    async def detect_cost_anomalies(
        self, account_id: str, threshold_percent: float = 30
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(datapoints) > 1:
                baseline = sum(d["Average"] for d in datapoints[:-1]) / len(
                    datapoints[:-1]
                )
    ...
    async def get_security_findings(
        self, account_id: str, severity: str = "CRITICAL"
    ) -> List[Dict[str, Any]]:
    ...
    async def get_performance_metrics(
        self, account_id: str, metric_name: str = "CPUUtilization"
    ) -> List[Dict[str, Any]]:
