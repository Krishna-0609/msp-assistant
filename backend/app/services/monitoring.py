...
class MonitoringService:
    ...
    async def send_teams_notification(self, alert: Dict[str, Any]) -> bool:
        ...
        if not settings.TEAMS_WEBHOOK_URL or "your-teams-webhook-url" in settings.TEAMS_WEBHOOK_URL:
        ...
        try:
                                    ...
                                    'items': [
                                        {
                                            ...
                                            'facts': [
                                                ...
                                                {
                                                    'name': 'Account:',
                                                    'value': alert.get('metadata', {}).get('account_id', 'N/A')
                                                }
                                            ]
                                        }
                                    ]
            ...
            if response.status_code in [200, 204]:
                logger.info(f"Teams notification sent successfully for {alert.get('type')}")
                return True
            else:
    ...
    async def monitor_performance(self) -> List[Dict[str, Any]]:
        ...
        metrics = await aws_service.get_performance_metrics(
            account_id='123456789012'
        )
